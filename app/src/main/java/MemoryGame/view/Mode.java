...
public class Mode extends JPanel implements MemoryUserInterface
{
	...
	public void Level(int level, Board board, MemoryController c)
	{
		this.board = board;
		this.controller = c;
		 switch (level){
       ...
       // sides.setOpaque(true);
		...
		for (int i = 0; i <numButtons; i++)
		{
			...
			card = new CardButton(i, board, cardFaces.get(k), face.frontCard, controller, true);
	...
	{
		int choice = JOptionPane.showConfirmDialog(mainFrame, "It Took You "+controller.AttemptCounter+ " Attempts \nDo You Want To Play Again?", "Results", JOptionPane.YES_NO_OPTION);
